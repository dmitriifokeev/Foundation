...
export default function ProgramCard({ course }) {
  ...
  // Извлекаем подкурсы и ценовые данные из details
  const { programCourses = [], price, oldPrice } = details;
  ...
  return (
    <Link to={`/courses/${slug}`}>
      <div className="relative w-full overflow-hidden group rounded-xl">
          ...
          <div className="absolute z-30 px-16 pt-8 mb-8 transition-opacity duration-300 md:group-hover:opacity-0">
            <div className="p-16 rounded-md bg-neutral-50">
              <div className="flex items-center gap-2 mb-[2px]">
                ...
                <span className={`body-14 font-medium px-3 rounded-full ${levelClasses}`}>
                  {tag}
                </span>
              </div>
              <h2 className="font-bold h5 text-neutral-800">{title}</h2>
            </div>
          </div>
        ...
        {/* Выплывающий блок (с информацией о подкурсах и цене) */}
        <div
        ...
        >
          ...
          >
            {programCourses.slice(0, 3).map((item, idx) => (
              <div key={idx} className="p-8 border-r border-neutral-600">
                ...
                <p className="leading-relaxed body-14 text-neutral-300">
                  {item.lessons} &middot; {item.hours} &middot; {item.duration} &middot;{" "}
            ...
            {price ? (
              <div className="relative flex flex-col items-center justify-center p-8">
                <div className="text-center">
                  <div className="mb-1 text-xl font-bold text-primary-400">{price}</div>
                  <div className="text-base line-through text-neutral-400">{oldPrice}</div>
                </div>
              </div>
            ) : (
